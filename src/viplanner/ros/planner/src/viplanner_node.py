...
ROS_TO_ROBOTICS_MAT = stf.Rotation.from_euler("XYZ", [0, 0, 0], degrees=True).as_matrix()
...
class VIPlannerNode:
    ...
    def __init__(self, cfg):
        ...
        print(f"depth topic: {self.cfg.depth_topic}")
        rospy.Subscriber(
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            # print(f"ready_for_planning_depth: {self.ready_for_planning_depth}")
            # print(f"ready_for_planning_rgb_sem: {self.ready_for_planning_rgb_sem}")
            # print(f"is_goal_init: {self.is_goal_init}")
            # print(f"goal_cam_frame_set: {self.goal_cam_frame_set}")
            if all(
                (
                ...
                if self.vip_algo.train_cfg.sem or self.vip_algo.train_cfg.rgb:
                    ...
                    print("sem waypoints shape:", waypoints.shape)
                else:
                    waypoints, fear = self.vip_algo.plan_depth(cur_depth_image, goal_cam_frame)
                    print("depth waypoints shape:", waypoints.shape)
    ...
    def imageCallback(self, rgb_msg: Image):
        ...
        if not success:
            print(f"Failed to get pose for rgb image at time {rgb_msg.header.stamp.to_sec()}")
            return
    ...
    def depthCallback(self, depth_msg: Image):
        ...
        if not success:
            print(f"Failed to get pose for depth image at time {depth_msg.header.stamp.to_sec()}")
            return
...
if __name__ == "__main__":
    ...
    # frame_ids
    parser.add_argument("robot_id", type=str, default="body", help="robot TF frame id")
    parser.add_argument("world_id", type=str, default="camera_init", help="world TF frame id")
