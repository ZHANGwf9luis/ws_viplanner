  ...
  while (status)
  {
    ...
    if (pathInit)
    {
      ...
      if (pubSkipCount < 0)
      {
        ...
        const double linear_limit = 0.23;
        const double angular_limit = 0.1;
        if (cmd_vel.linear.x > linear_limit)
          cmd_vel.linear.x = linear_limit;
        else if (cmd_vel.linear.x < -linear_limit)
          cmd_vel.linear.x = -linear_limit;

        if (cmd_vel.angular.z > angular_limit)
          cmd_vel.angular.z = angular_limit;
        else if (cmd_vel.angular.z < -angular_limit)
          cmd_vel.angular.z = -angular_limit;
